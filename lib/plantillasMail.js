...
function plantillaBienvenidaAuto(datos) {
  var nombre = datos.clienteNombre || "su equipo";
  var guiaUrl = datos.guiaUrl || (datos.portalUrl || PORTAL_URL_DEFAULT).replace(/\/$/, "") + "/guia";
  var subject = "Bienvenidos a Nubceo — su guía de autoimplementación";
  var cuerpo = "<p>Estimados/as,</p>"
    + "<p>Es un placer darles la bienvenida al proceso de autoimplementación de <b>Nubceo Conciliador</b> para <b>" + esc(nombre) + "</b>.</p>"
    + "<p>A diferencia de la implementación asistida, en este caso <b>ustedes configuran Nubceo a su ritmo</b>, siguiendo una guía paso a paso con videos y herramientas. El equipo de Nubceo sigue disponible para consultas puntuales.</p>"
    + bloqueAcceso(datos)
    + "<p>Desde la guía van a poder:</p>"
    + "<ul style=\"line-height:1.8; color:#1e2433;\">"
    + "<li>Crear sus sucursales y mapear los números de comercio.</li>"
    + "<li>Configurar cómo llegan las ventas (por CSV o por API).</li>"
    + "<li>Armar las reglas de conciliación y ejecutar la primera prueba.</li>"
    + "<li>Seguir su progreso paso a paso, con plazos y recordatorios.</li>"
    + "</ul>"
    + "<div style=\"margin:22px 0;\"><a href=\"" + esc(guiaUrl) + "\" style=\"display:inline-block; background:#6d28d9; color:#fff; text-decoration:none; padding:10px 20px; border-radius:8px; font-weight:600; font-size:14px;\">🚀 Abrir la guía de autoimplementación &rarr;</a></div>"
    + bloqueContactos(datos)
    + "<p style=\"margin-top:20px;\">Ante cualquier consulta, quedamos a su entera disposición.</p>"
    + firma();
  return { subject: subject, html: envoltorio(cuerpo) };
}

function plantillaRecordatorioAuto(datos) {
  var nombre = datos.clienteNombre || "su equipo";
  var paso = datos.pasoNombre ? (" del paso \u00ab" + esc(datos.pasoNombre) + "\u00bb") : "";
  var subject = "Recordatorio — avancen con su autoimplementación de Nubceo" + (datos.pasoNombre ? (" (\u00ab" + datos.pasoNombre + "\u00bb)") : "");
  var cuerpo = "<p>Estimados/as,</p>"
    + "<p>Nos comunicamos desde Nubceo para recordarles que la autoimplementación del Conciliador de <b>" + esc(nombre) + "</b> tiene pasos pendientes" + paso + (datos.fechaLimiteTxt ? (", con fecha límite <b>" + esc(datos.fechaLimiteTxt) + "</b>") : "") + ".</p>"
    + "<p>Les solicitamos retomar la guía a la brevedad para no perder el ritmo. Pueden ingresar con el código de acceso que les compartimos oportunamente.</p>"
    + bloqueAcceso(datos)
    + bloqueContactos(datos)
    + "<p style=\"margin-top:20px;\">Si surgió algún inconveniente que dificulte avanzar, les pedimos que nos lo hagan saber. El equipo de Nubceo sigue disponible para acompañarlos.</p>"
    + firma();
  return { subject: subject, html: envoltorio(cuerpo) };
}

function plantillaVencidoAuto(datos) {
  var nombre = datos.clienteNombre || "su equipo";
  var paso = datos.pasoNombre ? (" \u00ab" + esc(datos.pasoNombre) + "\u00bb") : "";
  var subject = "Plazo vencido — autoimplementación de Nubceo" + (datos.pasoNombre ? (" (\u00ab" + datos.pasoNombre + "\u00bb)") : "");
  var cuerpo = "<p>Estimados/as,</p>"
    + "<p>Les escribimos para informarles que el plazo establecido para completar el paso" + paso + " de la autoimplementación del Conciliador de <b>" + esc(nombre) + "</b>" + (datos.fechaLimiteTxt ? (" <b>venció el " + esc(datos.fechaLimiteTxt) + "</b>") : " se encuentra vencido") + " y, a la fecha, continúa pendiente.</p>"
    + "<p>Les pedimos retomar la guía a la brevedad. Si necesitan asistencia, no duden en contactarnos.</p>"
    + bloqueAcceso(datos)
    + bloqueContactos(datos)
    + "<p style=\"margin-top:20px;\">Quedamos a disposición para acompañarlos y encontrar una solución.</p>"
    + firma();
  return { subject: subject, html: envoltorio(cuerpo) };
}
...
const _UI = {
  ...
  bienvenida_auto: plantillaBienvenidaAuto,
  recordatorio_auto: plantillaRecordatorioAuto,
  vencido_auto: plantillaVencidoAuto,
};
...
export const PLANTILLAS_DISPONIBLES = [
  ...
  { id: "bienvenida_auto", nombre: "Bienvenida autoimplementación 🚀" },
  { id: "recordatorio_auto", nombre: "Recordatorio autoimplementación 🚀" },
  { id: "vencido_auto", nombre: "Plazo vencido autoimplementación 🚀" },
];
